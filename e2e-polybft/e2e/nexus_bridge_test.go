...
func TestE2E_ApexBridgeWithNexus_SrcNexus_InvalidScenarios(t *testing.T) {
	...
	for _, dstChain := range dstChains {
		...
		t.Run("Big receiver amount", func(t *testing.T) {
			...
			_, err = apex.SubmitTx(
				ctx, srcChain, nexusAdminUser, unfundedUser.NexusAddress.String(), cardanofw.ApexToDfm(big.NewInt(10)), nil, nil)
			require.NoError(t, err)
...
func TestE2E_ApexBridgeWithNexus_BatchFailed(t *testing.T) {
	...
	t.Run("Test insufficient gas price dynamicTx=true", func(t *testing.T) {
		...
		txHash, err := apex.SubmitBridgingRequest(
			ctx, srcChain, cardanofw.ChainIDNexus, user, sendAmountDfm, user)
		require.NoError(t, err)
...
func TestE2E_ApexBridgeWithNexus_NexusFundAmount(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			txHash, err := apex.SubmitBridgingRequest(
				ctx, tc.fromChain, tc.toChain, user, tc.sendAmountDfm, user)
			require.NoError(t, err)
...
func TestE2E_ApexBridgeWithNexus_PrimeGoesDownAndThenUp(t *testing.T) {
	...
	txHash, err := apex.SubmitBridgingRequest(ctx, cardanofw.ChainIDNexus, cardanofw.ChainIDPrime, user, sendAmountDfm, user)
	require.NoError(t, err)
...
func TestE2E_ApexBridgeWithNexus_NexusGoesDownAndThenUp(t *testing.T) {
	...
	txHash, err := apex.SubmitBridgingRequest(ctx, cardanofw.ChainIDPrime, cardanofw.ChainIDNexus, user, sendAmountDfm, user)
	require.NoError(t, err)
