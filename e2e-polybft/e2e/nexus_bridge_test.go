...
func TestE2E_ApexBridgeWithNexus_NtP_ValidScenarios(t *testing.T) {
	...
	t.Run("From Nexus to Prime - sequential and parallel multiple receivers", func(t *testing.T) {
		...
		NexusToPrimeSequentialAndParallelWithMaxReceivers(
			t, ctx, apex, instances, parallelInstances, sendAmountDfm)
	})
...
func TestE2E_ApexBridgeWithNexus_PtN_InvalidScenarios(t *testing.T) {
	...
	t.Run("Submitter not enough funds", func(t *testing.T) {
		...
		PrimeToNexusSubmitterNotEnoughFunds(t, ctx, apex, user, sendAmountDfm, bridgingFeeAmount)
	})

	t.Run("Submitted invalid metadata", func(t *testing.T) {
		sendAmountDfm := cardanofw.WeiToDfm(ethgo.Ether(1))

		PrimeToNexusInvalidMetadataSlicedOff(t, ctx, apex, user, sendAmountDfm, bridgingFeeAmount)
	})

	t.Run("Submitted invalid metadata - wrong type", func(t *testing.T) {
		sendAmountDfm := cardanofw.WeiToDfm(ethgo.Ether(1))

		PrimeToNexusInvalidMetadataWrongType(t, ctx, apex, user, cardanofw.DefaultRequestStateTimeoutSec, sendAmountDfm, bridgingFeeAmount)
	})

	t.Run("Submitted invalid metadata - invalid destination", func(t *testing.T) {
		sendAmountDfm := cardanofw.WeiToDfm(ethgo.Ether(1))

		PrimeToNexusInvalidMetadataInvalidDestination(t, ctx, apex, user, 0, sendAmountDfm, bridgingFeeAmount)
	})

	t.Run("Submitted invalid metadata - invalid sender", func(t *testing.T) {
		sendAmountDfm := cardanofw.WeiToDfm(ethgo.Ether(1))

		PrimeToNexusInvalidMetadataInvalidSender(t, ctx, apex, user, 0, sendAmountDfm, bridgingFeeAmount)
	})

	t.Run("Submitted invalid metadata - empty tx", func(t *testing.T) {
		sendAmountDfm := cardanofw.WeiToDfm(ethgo.Ether(1))

		PrimeToNexusInvalidMetadataInvalidTransactions(t, ctx, apex, user, 0, sendAmountDfm, bridgingFeeAmount)
	})
}
...
func TestE2E_ApexBridgeWithNexus_NexusGoesDownAndThenUp(t *testing.T) {
	...
	txHash := apex.SubmitBridgingRequest(t, ctx, cardanofw.ChainIDPrime, cardanofw.ChainIDNexus, user, sendAmountDfm, sendtx.BridgingTypeNormal, user)
