...
func TestE2E_ApexBridge_Over_Max_Allowed_To_Bridge(t *testing.T) {
	...
	for idx, br := range bridgingRequests {
		...
		go func() {
			...
			err := apex.WaitForAmountInRange(ctx, apex.Users[0], br.src, lowerBoundaryDfm, beforeSendingAmountDfm[idx][infrawallet.AdaTokenName],
...
func TestE2E_ApexBridge_ValidScenarios(t *testing.T) {
	...
	t.Run("Submitter has tokens", func(t *testing.T) {
		...
		e2ehelper.ExecuteSingleBridging(
			t, ctx, apex, brSubmitterUser, user, cardanofw.ChainIDPrime, cardanofw.ChainIDVector, sendAmountWei,
	...
	t.Run("From prime to vector one by one", func(t *testing.T) {
		...
		sendAmount := ethgo.Gwei(1_000_005_000)
	...
	receivers := []sendtx.BridgingTxReceiver{
		{
			...
			Amount:  cardanofw.WeiToDfm(sendAmount).Uint64(), // TODO: this probably will be big.Int
			TokenID: tokensInfo.SrcTokenID,
		},
	}
