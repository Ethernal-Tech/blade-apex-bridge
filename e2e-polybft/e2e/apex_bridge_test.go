...
func TestE2E_SkylineUTxOConsolidation(t *testing.T) {
	...
	primeConfig.FundUTxOCount = fundUtxoCount
	primeConfig.FundAmount = cardanofw.MinUTxODefaultValue * fundUtxoCount
	...
	sendAmount := primeConfig.FundTokenAmount - cardanofw.MinUTxODefaultValue*3
	...
	apex := cardanofw.SetupAndRunApexBridge(
		t, ctx, cardanofw.SystemIDSkyline,
		cardanofw.WithUserCnt(1),
		cardanofw.WithPrimeConfig(primeConfig),
		cardanofw.WithCustomConfigHandlers(func(a *cardanofw.ApexSystem, mp map[string]any) {
			t.Helper()
	...
	_, err = cardanofw.FundUserWithToken(
		ctx, cardanofw.ChainIDPrime, apex.Config.PrimeConfig.NetworkType, txProviderPrime,
		minterWalletPrime, apex.Users[0], uint64(1_100_000_000), uint64(5_000_000))
	require.NoError(t, err)
	...
	require.Equal(t, uint64(0), getLastConfirmedBatchID(cardanofw.ChainIDPrime))

	utxos, err := txProviderPrime.GetUtxos(ctx, apex.PrimeInfo.MultisigAddr)
	require.NoError(t, err)

	require.Len(t, utxos, 2*primeConfig.FundUTxOCount)
	e2ehelper.ExecuteSingleBridging(
		t, ctx, apex, apex.Users[0], apex.Users[0],
		cardanofw.ChainIDPrime, cardanofw.ChainIDCardano, new(big.Int).SetUint64(sendAmount), sendtx.BridgingTypeNativeTokenOnSource)

	require.Equal(t, uint64(2), getLastConfirmedBatchID(cardanofw.ChainIDPrime))
