	...
	for chainKey, prevAmount := range chainPrevAmounts {
		...
		go func() {
			...
			err = apex.WaitForExactAmount(
				ctx, chainReceivers[chainKey], chainKey.chain, chainKey.destChain, expectedAmount, numRetries, waitTime)
...
func TestE2E_ApexBridgeWithNexus_PrimeGoesDownAndThenUp(t *testing.T) {
	...
	txHash := apex.SubmitBridgingRequest(
		t, ctx, cardanofw.ChainIDNexus, cardanofw.ChainIDPrime, user, sendAmountDfm, sendtx.BridgingTypeNormal, user)
	...
	expectedAmountDfm := new(big.Int).Add(prevAmountPrimeDfm[infrawallet.AdaTokenName], sendAmountDfm)
