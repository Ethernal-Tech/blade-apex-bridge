...
func TestE2E_Apex_Bridge_Refund_ValidScenarios(t *testing.T) {
	...
	t.Run("From prime to vector - not enough funds on destination multisig address", func(t *testing.T) {
		...
		metadata := map[string]interface{}{
			"1": map[string]interface{}{
				"t": "bridge",
				"d": cardanofw.ChainIDVector,
				"s": cardanofw.AddrToMetaDataAddr(brSubmitterUser.GetAddress(cardanofw.ChainIDPrime)),
				"tx": []cardanofw.BridgingRequestMetadataTransaction{{
					Address: cardanofw.AddrToMetaDataAddr(brSubmitterUser.GetAddress(cardanofw.ChainIDVector)),
					Amount:  sendAmount,
				}},
			},
		}
		...
		lowerBoundaryDfm := new(big.Int).Sub(beforeSendingAmountDfm, new(big.Int).SetUint64(sendAmount))

		fmt.Println("lowerBoundaryDfm", lowerBoundaryDfm)
		fmt.Println("higherBoundaryDfm", beforeSendingAmountDfm)
	...
	t.Run("Submitted invalid metadata - wrong type", func(t *testing.T) {
		...
		fmt.Println("lowerBoundaryDfm", lowerBoundaryDfm)
		fmt.Println("higherBoundaryFfm", beforeSendingAmountDfm)
	...
	t.Run("Submitted invalid metadata - empty tx", func(t *testing.T) {
		...
		cardanofw.WaitForInvalidState(t, ctx, apex, cardanofw.ChainIDPrime, txHash, apex.Config.APIKey, 30)
	...
	t.Run("Submitted invalid metadata - invalid sender", func(t *testing.T) {
		...
		metadata := map[string]interface{}{
			"1": map[string]interface{}{
				"t":  "bridge",
				"s":  []string{"adsasdas"}, // should be sender address (max len 40)
				"d":  cardanofw.ChainIDVector,
				"tx": transactions,
				"fa": feeAmount,
			},
	...
	t.Run("Submitted not enough funds - invalid", func(t *testing.T) {
		...
		err = apex.WaitForGreaterAmount(ctx, user, cardanofw.ChainIDPrime, lowerBoundaryDfm,
			10, time.Second*10)
		require.Error(t, err)
	})
