...
const (
	...
	defaultPremineAmount   = uint64(5_000_000)
)
...
func NewTestCardanoChain(config *TestCardanoChainConfig) ITestApexChain {
	if !config.IsEnabled {
		...
		return NewTestApexChainDummy([]string{
			getFlag("network-address"), "http://localhost:1000",
			getFlag("network-magic"), fmt.Sprint(GetNetworkMagic(config.NetworkType)),
			getFlag("network-id"), GetNetworkName(config.NetworkType),
...
func (ec *TestCardanoChain) FundWallets(ctx context.Context) error {
	...
	fund := func(
		ctx context.Context, cluster *TestCardanoCluster, addr string,
	) (string, error) {
		txProvider := cardWallet.NewTxProviderOgmios(cluster.OgmiosURL())
		...
		txHash, err := SendTx(ctx, txProvider, genesisWallet, ec.config.FundAmount,
			addr, ec.config.NetworkType, []byte{})
		if err != nil {
			return "", err
		}
	...
	txHash, err := fund(ctx, ec.cluster, ec.multisigAddr)
	if err != nil {
		return err
	}
	...
	txHash, err = fund(ctx, ec.cluster, ec.multisigFeeAddr)
	if err != nil {
		return err
	}
...
func (ec *TestCardanoChain) PopulateApexSystem(apexSystem *ApexSystem) {
	switch ec.ChainID() {
	case ChainIDPrime:
		apexSystem.PrimeMultisigAddr = ec.multisigAddr
		apexSystem.PrimeMultisigFeeAddr = ec.multisigFeeAddr
		apexSystem.PrimeInfo = CardanoChainInfo{
			NetworkAddress: ec.cluster.Servers[0].NetworkAddress(),
			OgmiosURL:      ec.cluster.OgmiosURL(),
		}
	case ChainIDVector:
		apexSystem.VectorMultisigAddr = ec.multisigAddr
		apexSystem.VectorMultisigFeeAddr = ec.multisigFeeAddr
