...
func (ec *TestEVMChain) InitContracts(bridgeAdmin *crypto.ECDSAKey, bridgeURL string) error {
	...
	var (
		...
		params = []string{
			"deploy-evm",
			"--url", ec.cluster.Servers[0].BridgeJSONRPCAddr(),
...
func (ec *TestEVMChain) PopulateApexSystem(apexSystem *ApexSystem) {
	if ec.config.ChainID == ChainIDNexus {
		apexSystem.nexusNode = ec.cluster.Servers[0]
		apexSystem.nexusGatewayAddress = ec.gatewayAddr
		apexSystem.nexusRelayerAddress = ec.relayerWallet.Address()
	}
}
