...
func TestGovernanceStore_InsertAndGetEvents(t *testing.T) {
	...
	events := []contractsapi.EventAbi{
		&contractsapi.NewValidatorWhitelistEvent{Validator: types.StringToAddress("0xDA01")},
		&contractsapi.NewSprintSizeEvent{Size: big.NewInt(10)},
		&contractsapi.NewValidatorWhitelistEvent{Validator: types.StringToAddress("0xDA02")},
		&contractsapi.NewValidatorWhitelistEvent{Validator: types.StringToAddress("0xDA03")},
	}
	...
	eventsRaw, err = state.GovernanceStore.
		getGovernanceEventsByType(201, (&contractsapi.NewValidatorWhitelistEvent{}).Sig(), nil)

	require.Nil(t, eventsRaw, nil)
	require.NoError(t, err)

	eventsRaw, err = state.GovernanceStore.
		getGovernanceEventsByType(200, (&contractsapi.NewValidatorWhitelistEvent{}).Sig(), nil)
