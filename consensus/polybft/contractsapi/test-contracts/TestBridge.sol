...
contract TestBridge is IBridge, Utils, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function submitNewValidatorSet(NewValidatorSetDelta calldata _newValidatorSetDelta) external override onlySystem {
        ValidatorSet[] calldata deltaAdded = _newValidatorSetDelta.addedValidators;
        address[] calldata deltaRemoved = _newValidatorSetDelta.removedValidators;
        BridgeValidatorsData[] memory addedValidators = 
            new BridgeValidatorsData[](deltaAdded.length);
        for (uint256 i = 0; i < deltaAdded.length; i++) {
            ValidatorSet calldata v = deltaAdded[i];

            addedValidators[i].chainID = v.chainId;
            addedValidators[i].validatorData = new ValidatorData[](v.validators.length);
            for (uint256 j = 0; j < v.validators.length; j++) {
                ValidatorAddressChainData calldata va = v.validators[i];
                addedValidators[i].validatorData[i] = 
                    ValidatorData(va.addr, va.data.key, va.keySignature, va.keyFeeSignature);
            }
        }

        ValidatorDelta memory vd = ValidatorDelta(addedValidators, deltaRemoved);
        ...
        if(!success) {
            revert("NO SUCCESS CALLING BLADE STAKE MANAGER");
        }
